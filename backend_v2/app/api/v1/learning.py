...
# ═══════════════════════════════════════
#  LEARNING FOREST (SKILL TREE)
# ═══════════════════════════════════════

@router.get("/tree")
def get_learning_tree(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    Fetch the visual learning forest nodes and prerequisites with real user progress.
    """
    nodes = db.query(SkillNode).all()
    
    # Get user's completed courses
    completed_payments = db.query(CoursePayment).filter(
        CoursePayment.user_rid == current_user.rid,
        CoursePayment.status == "completed"
    ).all()
    completed_course_ids = {p.course_id for p in completed_payments}
    
    # Map SkillNode ID to its completion status for easy prereq checking
    node_completion_map = {}
    for n in nodes:
        node_completion_map[n.id] = n.course_id in completed_course_ids if n.course_id else False

    result = []
    for n in nodes:
        # Determine status
        if node_completion_map.get(n.id):
            status = "COMPLETED"
        else:
            # Check prerequisites
            all_prereqs_met = True
            for prereq in n.prerequisites:
                if not node_completion_map.get(prereq.id):
                    all_prereqs_met = False
                    break
            
            status = "UNLOCKED" if all_prereqs_met else "LOCKED"
        
        # Build the prerequisites list for the frontend
        prereqs = [{"required_node_id": p.id} for p in n.prerequisites]
