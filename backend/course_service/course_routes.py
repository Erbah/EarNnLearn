...
from common.models.learning import CoursePayment, VideoProgress, generate_uuid, get_now
from common.models.admin import Season, Tier
...
def enroll_paid_course(course_id: str, body: EnrollRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        if existing:
            logging.info(f"Enrollment for {current_user.email} already exists")
        ...
        logging.info(f"Enrolling {current_user.email} via earn-to-learn (RAW SQL FALLBACK)")
        payment_id = generate_uuid()
        db.execute(text("""
            INSERT INTO course_payments (id, user_rid, course_id, total_price, amount_paid, remaining, payment_method, status, ppc, start_season, created_at)
            VALUES (:id, :user_rid, :course_id, :total_price, :amount_paid, :remaining, :payment_method, :status, :ppc, :start_season, :created_at)
        """), {
            "id": payment_id, "user_rid": current_user.rid, "course_id": course_id,
            "total_price": float(price), "amount_paid": 0.0, "remaining": float(price),
            "payment_method": "earn_to_learn", "status": "active", "ppc": float(ppc),
            "start_season": current_season, "created_at": get_now()
        })
        
        enrollment_id = generate_uuid()
        db.execute(text("INSERT INTO course_enrollments (id, course_id, user_rid, enrolled_at) VALUES (:id, :cid, :urid, :at)"), 
                   {"id": enrollment_id, "cid": course_id, "urid": current_user.rid, "at": get_now()})
        
        db.execute(text("UPDATE courses SET enrollment_count = COALESCE(enrollment_count, 0) + 1 WHERE id = :cid"), {"cid": course_id})
        
        logging.info(f"VIM Trigger for user {current_user.email}, parent={current_user.parent_rid}")
        if current_user.parent_rid:
            parent_rid = current_user.parent_rid
            logging.info(f"Scholarship Trigger: Checking parent_rid={parent_rid} for course={course_id}")
            scholarship = db.query(CourseScholarship).filter(
                CourseScholarship.user_rid == parent_rid,
                CourseScholarship.course_id == course_id
            ).first()

            if not scholarship:
                logging.info(f"Creating new scholarship record for parent_rid={parent_rid}")
                scholarship = CourseScholarship(id=generate_uuid(), user_rid=parent_rid, course_id=course_id, referral_enrollment_count=0)
                db.add(scholarship)
                db.flush()
            
            if not scholarship.scholarship_active:
                scholarship.referral_enrollment_count += 1
                logging.info(f"Referral count for {parent_rid} now {scholarship.referral_enrollment_count}/5")
                if scholarship.referral_enrollment_count >= 5:
                    logging.info(f"AWARDING SCHOLARSHIP to {parent_rid} for course {course_id}")
                    scholarship.scholarship_active = True
                    scholarship.awarded_at = datetime.utcnow()
                    
                    db.execute(text("""
                        UPDATE course_payments 
                        SET status = 'completed', remaining = 0, payment_method = 'scholarship'
                        WHERE user_rid = :urid AND course_id = :cid
                    """), {"urid": parent_rid, "cid": course_id})
        ...
        logging.exception("Error in enroll_paid_course")
        db.rollback()
...
def watch_video(course_id: str, body: WatchVideoRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # 7. GAMIFICATION TRIGGER
    GamificationService.update_streak(db, current_user)
    GamificationService.award_xp(db, current_user, 50) # Standard 50 XP per lesson
