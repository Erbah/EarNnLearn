...
def generate_deep_lesson_scenes(db: Session, user_rid: str, topic: str, difficulty: str, education_level: str, learner_goal: str, style: str, uai: str | None = None, source_id: str | None = None):
    ...
    def generate_chapter(key):
        # Create a thread-local session for safety
        thread_db = SessionLocal()
        try:
            print(f"DEBUG: Parallel Streaming Chapter: {key} for topic: {topic}")
            ...
            print(f"ERROR in parallel generation for {key}: {str(e)}")
            return key, [] # Fallback to empty list or handled error
        finally:
            thread_db.close()
    ...
    with ThreadPoolExecutor(max_workers=len(section_keys)) as executor:
        futures = {executor.submit(generate_chapter, key): key for key in section_keys}
        try:
            ...
            # We still return what we have in results
    
    # Assemble in order to maintain pedagogical flow
    all_scenes = []
